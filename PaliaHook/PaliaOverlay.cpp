...
void PaliaOverlay::DrawOverlay()
{
	...
	std::string WindowTitle = std::string("OriginPalia Menu By Wimberton @ UnknownCheats###OriginPalia - Renderer: ") + (hook == nullptr ? "Unknown" : hook->GetLibName());
	...
	if (ImGui::Begin(WindowTitle.data(), &show, window_flags))
	{
		...
		if (OpenTab == 0) {
			...
			if (ImGui::CollapsingHeader("Player & Entities", ImGuiTreeNodeFlags_DefaultOpen))
			{
						...
						if (PlayerController && PlayerController->Pawn) {
							...
							if (ValeriaCharacter) {
								...
								if (ImGui::CollapsingHeader("Movement Settings - General", ImGuiTreeNodeFlags_DefaultOpen)) {
									...
									// ImGui::Checkbox("Enable Teleport to Map Marker", &bEnableMapTeleportToMarker);
									// if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled)) ImGui::SetTooltip("Automatically teleport to the map waypoint if you set one.");

...
bool DX9Hook::StartHook() {
  if (!hooked) {
    ...
    HookFuncs(
        ...
        std::make_pair<void **, void *>(&(PVOID &)Present,
                                        &DX9Hook::MyPresent));
    if (PresentEx != nullptr) {
      HookFuncs(std::make_pair<void **, void *>(&(PVOID &)PresentEx,
                                                &DX9Hook::MyPresentEx)
...
HRESULT STDMETHODCALLTYPE DX9Hook::MyReset(
    IDirect3DDevice9 *_this, D3DPRESENT_PARAMETERS *pPresentationParameters) {
...
HRESULT STDMETHODCALLTYPE DX9Hook::MyPresent(IDirect3DDevice9 *_this,
  ...
  DX9Hook::Instance()->PrepareForOverlay(_this);
  return (_this->*DX9Hook::Instance()->Present)(
      pSourceRect, pDestRect, hDestWindowOverride, pDirtyRegion);
}

HRESULT STDMETHODCALLTYPE DX9Hook::MyPresentEx(
  ...
  DX9Hook::Instance()->uses_present = true;
  DX9Hook::Instance()->PrepareForOverlay(_this);
  return (_this->*DX9Hook::Instance()->PresentEx)(
      pSourceRect, pDestRect, hDestWindowOverride, pDirtyRegion, dwFlags);
}

DX9Hook::DX9Hook()
    : initialized(false), hooked(false), uses_present(false), EndScene(nullptr),
      Present(nullptr), PresentEx(nullptr), Reset(nullptr) {

...
template <typename T> static inline void SafeRelease(T *&pUnk) {
  if (pUnk != nullptr) {
    pUnk->Release();
    pUnk = nullptr;
  }
...
ID3D12CommandQueue *
DX12Hook::FindCommandQueueFromSwapChain(IDXGISwapChain *pSwapChain) {
  ID3D12CommandQueue *pCommandQueue = nullptr;

  if (CommandQueueOffset == 0 && pCmdQueue != nullptr) {
    for (size_t i = 0; i < 1024; ++i) {
      if (*reinterpret_cast<ID3D12CommandQueue **>(
              reinterpret_cast<uintptr_t>(pSwapChain) + i) == pCmdQueue) {
  ...
  if (CommandQueueOffset != 0)
    pCommandQueue = *reinterpret_cast<ID3D12CommandQueue **>(
        reinterpret_cast<uintptr_t>(pSwapChain) + CommandQueueOffset);
...
bool DX12Hook::StartHook() {
  ...
  if (!hooked) {
    ...
    BeginHook();
    HookFuncs(
        std::make_pair<void **, void *>(&(PVOID &)DX12Hook::Present,
                                        &DX12Hook::MyPresent),
        std::make_pair<void **, void *>(&(PVOID &)DX12Hook::ResizeTarget,
                                        &DX12Hook::MyResizeTarget),
        std::make_pair<void **, void *>(&(PVOID &)DX12Hook::ResizeBuffers,
                                        &DX12Hook::MyResizeBuffers),
        std::make_pair<void **, void *>(&(PVOID &)DX12Hook::ExecuteCommandLists,
                                        &DX12Hook::MyExecuteCommandLists));
    if (Present1 != nullptr) {
      HookFuncs(std::make_pair<void **, void *>(&(PVOID &)Present1,
                                                &DX12Hook::MyPresent1));
    }
    EndHook();
...
// affect game's fps.
void DX12Hook::PrepareForOverlay(IDXGISwapChain *pSwapChain,
  ...
  if (!initialized) {
...

    {
      ...
      if (pDevice->CreateDescriptorHeap(&desc, IID_PPV_ARGS(&pSrvDescHeap)) !=
          S_OK) {
        pDevice->Release();
        pSwapChain3->Release();
        return;
...

    {
      ...
      desc.NodeMask = 1;
      if (pDevice->CreateDescriptorHeap(&desc, IID_PPV_ARGS(&pRtvDescHeap)) !=
          S_OK) {
        pDevice->Release();
        pSwapChain3->Release();
      ...
      for (UINT i = 0; i < bufferCount; ++i) {
        ...
        if (pSwapChain3->GetBuffer(i, IID_PPV_ARGS(&pBackBuffer)) != S_OK ||
            pBackBuffer == nullptr) {
          OverlayFrames.clear();
          pCmdList->Release();
          pCmdAlloc->Release();
  ...
  if (ImGui_ImplDX12_NewFrame()) {
    ...
    pCmdList->OMSetRenderTargets(1, &OverlayFrames[bufferIndex].RenderTarget,
                                 FALSE, NULL);
    ...
    pCommandQueue->ExecuteCommandLists(1,
                                       (ID3D12CommandList *const *)&pCmdList);
  }
  pSwapChain3->Release();
}

HRESULT STDMETHODCALLTYPE DX12Hook::MyPresent(IDXGISwapChain *_this,
                                              UINT SyncInterval, UINT Flags) {
  ID3D12CommandQueue *pCommandQueue =
      DX12Hook::Instance()->FindCommandQueueFromSwapChain(_this);
  if (pCommandQueue != nullptr) {
...
HRESULT STDMETHODCALLTYPE DX12Hook::MyResizeTarget(
...
HRESULT STDMETHODCALLTYPE DX12Hook::MyResizeBuffers(IDXGISwapChain *_this,
                                                    UINT BufferCount,
                                                    UINT Width, UINT Height,
                                                    DXGI_FORMAT NewFormat,
                                                    UINT SwapChainFlags) {
  DX12Hook::Instance()->ResetRenderState();
  return (_this->*DX12Hook::Instance()->ResizeBuffers)(
      BufferCount, Width, Height, NewFormat, SwapChainFlags);
}

void STDMETHODCALLTYPE
DX12Hook::MyExecuteCommandLists(ID3D12CommandQueue *_this, UINT NumCommandLists,
                                ID3D12CommandList *const *ppCommandLists) {
  DX12Hook *me = DX12Hook::Instance();
  me->pCmdQueue = _this;

  (_this->*DX12Hook::Instance()->ExecuteCommandLists)(NumCommandLists,
                                                      ppCommandLists);
}

HRESULT STDMETHODCALLTYPE
DX12Hook::MyPresent1(IDXGISwapChain1 *_this, UINT SyncInterval, UINT Flags,
  ...
  return (_this->*DX12Hook::Instance()->Present1)(SyncInterval, Flags,
                                                  pPresentParameters);
}

DX12Hook::DX12Hook()
    : hooked(false), initialized(false), CommandQueueOffset(0),
      pCmdQueue(nullptr), pDevice(nullptr), pSrvDescHeap(nullptr),
      pCmdList(nullptr), pRtvDescHeap(nullptr), Present(nullptr),
      ResizeBuffers(nullptr), ResizeTarget(nullptr),
      ExecuteCommandLists(nullptr), Present1(nullptr) {
...
const char *DX12Hook::GetLibName() const { return DX12_DLL; }

void DX12Hook::LoadFunctions(ID3D12CommandQueue *pCommandQueue,

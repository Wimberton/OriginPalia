...
ID3D12CommandQueue *DX12Hook::FindCommandQueueFromSwapChain(IDXGISwapChain *pSwapChain) {
    ...
    if (CommandQueueOffset != 0)
        pCommandQueue =
            *reinterpret_cast<ID3D12CommandQueue **>(reinterpret_cast<uintptr_t>(pSwapChain) + CommandQueueOffset);
...
bool DX12Hook::StartHook() {
    ...
    if (!hooked) {
        ...
        if (Present1 != nullptr) {
            HookFuncs(std::make_pair<void **, void *>(&(PVOID &)Present1, &DX12Hook::MyPresent1));
        }
        EndHook();
    ...
    if (!initialized) {
...

        {
            ...
            for (UINT i = 0; i < bufferCount; ++i) {
                ...
                if (pDevice->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(&pCmdAlloc)) != S_OK ||
                    pCmdAlloc == nullptr) {
                    OverlayFrames.clear();
                    pSrvDescHeap->Release();
                ...
                if (i == 0) {
                    if (pDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, pCmdAlloc, NULL,
...
HRESULT STDMETHODCALLTYPE DX12Hook::MyResizeBuffers(IDXGISwapChain *_this, UINT BufferCount, UINT Width, UINT Height,
                                                    DXGI_FORMAT NewFormat, UINT SwapChainFlags) {
    DX12Hook::Instance()->ResetRenderState();
    return (_this->*DX12Hook::Instance()->ResizeBuffers)(BufferCount, Width, Height, NewFormat, SwapChainFlags);
}

void STDMETHODCALLTYPE DX12Hook::MyExecuteCommandLists(ID3D12CommandQueue *_this, UINT NumCommandLists,
...
HRESULT STDMETHODCALLTYPE DX12Hook::MyPresent1(IDXGISwapChain1 *_this, UINT SyncInterval, UINT Flags,
...
DX12Hook::DX12Hook()
    : hooked(false), initialized(false), CommandQueueOffset(0), pCmdQueue(nullptr), pDevice(nullptr),
      pSrvDescHeap(nullptr), pCmdList(nullptr), pRtvDescHeap(nullptr), Present(nullptr), ResizeBuffers(nullptr),
      ResizeTarget(nullptr), ExecuteCommandLists(nullptr), Present1(nullptr) {

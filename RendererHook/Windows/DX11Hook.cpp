...
bool DX11Hook::StartHook() {
  ...
  if (!hooked) {
    ...
    BeginHook();
    HookFuncs(std::make_pair<void **, void *>(&(PVOID &)DX11Hook::Present,
                                              &DX11Hook::MyPresent),
              std::make_pair<void **, void *>(&(PVOID &)DX11Hook::ResizeTarget,
                                              &DX11Hook::MyResizeTarget),
              std::make_pair<void **, void *>(&(PVOID &)DX11Hook::ResizeBuffers,
                                              &DX11Hook::MyResizeBuffers));
    EndHook();
...
void DX11Hook::PrepareForOverlay(IDXGISwapChain *pSwapChain) {
  ...
  if (!initialized) {
    ...
    ID3D11RenderTargetView
        *get_targets[D3D11_SIMULTANEOUS_RENDER_TARGET_COUNT] = {};
    pContext->OMGetRenderTargets(D3D11_SIMULTANEOUS_RENDER_TARGET_COUNT,
                                 get_targets, NULL);
...
HRESULT STDMETHODCALLTYPE DX11Hook::MyPresent(IDXGISwapChain *_this,
                                              UINT SyncInterval, UINT Flags) {
  DX11Hook::Instance()->PrepareForOverlay(_this);
  return (_this->*DX11Hook::Instance()->Present)(SyncInterval, Flags);
}

HRESULT STDMETHODCALLTYPE DX11Hook::MyResizeTarget(
...
HRESULT STDMETHODCALLTYPE DX11Hook::MyResizeBuffers(IDXGISwapChain *_this,
                                                    UINT BufferCount,
                                                    UINT Width, UINT Height,
                                                    DXGI_FORMAT NewFormat,
                                                    UINT SwapChainFlags) {
  DX11Hook::Instance()->ResetRenderState();
  return (_this->*DX11Hook::Instance()->ResizeBuffers)(
      BufferCount, Width, Height, NewFormat, SwapChainFlags);
}

DX11Hook::DX11Hook()
    : initialized(false), hooked(false), pContext(nullptr),

...
void RendererDetector::CreateHWnd() {
    if (DummyHWnd == nullptr) {
        ...
        if (Atom > 0) {
            DummyHWnd = ::CreateWindowEx(NULL, windowClassName, "", WS_OVERLAPPEDWINDOW, 0, 0, 1, 1, NULL, NULL, hInst,
                                         nullptr);
...
HRESULT WINAPI RendererDetector::MyPresent(IDirect3DDevice9 *_this, const RECT *pSourceRect, const RECT *pDestRect,
...
HRESULT WINAPI RendererDetector::MyPresentEx(IDirect3DDevice9Ex *_this, const RECT *pSourceRect, const RECT *pDestRect,
                                             HWND hDestWindowOverride, const RGNDATA *pDirtyRegion, DWORD dwFlags) {
    RendererDetector &inst = RendererDetector::Instance();
    HookManager &hm = HookManager::Instance();
    auto res =
        (_this->*_IDirect3DDevice9Ex_PresentEx)(pSourceRect, pDestRect, hDestWindowOverride, pDirtyRegion, dwFlags);
    if (!inst.StopRetry()) {
        DX9Hook::Instance()->StartHook();
    }
    return res;
...
void RendererDetector::HookDXGIPresent(IDXGISwapChain *pSwapChain) {
    if (!bDXGIHooked) {
        bDXGIHooked = true;
        (void *&)_IDXGISwapChain_Present =
            (*reinterpret_cast<void ***>(pSwapChain))[(int)IDXGISwapChainVTable::Present];

        RenderDetectHook->BeginHook();
        RenderDetectHook->HookFuncs(
            std::pair<void **, void *>((PVOID *)&_IDXGISwapChain_Present, &RendererDetector::MyIDXGISwapChain_Present));
...
void RendererDetector::HookDX9Present(IDirect3DDevice9 *pDevice, bool ex) {
    ...
    if (ex)
        (void *&)_IDirect3DDevice9Ex_PresentEx =
            (*reinterpret_cast<void ***>(pDevice))[(int)IDirect3DDevice9VTable::PresentEx];

    RenderDetectHook->BeginHook();
    RenderDetectHook->HookFuncs(
        std::pair<void **, void *>((PVOID *)&_IDirect3DDevice9_Present, &RendererDetector::MyPresent));
    if (ex)
        RenderDetectHook->HookFuncs(
            std::pair<void **, void *>((PVOID *)&_IDirect3DDevice9Ex_PresentEx, &RendererDetector::MyPresentEx));
    RenderDetectHook->EndHook();
}
...
void RendererDetector::HookwglMakeCurrent(decltype(wglMakeCurrent) *wglMakeCurrent) {
    ...
    RenderDetectHook->BeginHook();
    RenderDetectHook->HookFuncs(
        std::pair<void **, void *>((PVOID *)&_wglMakeCurrent, &RendererDetector::MywglMakeCurrent));
    RenderDetectHook->EndHook();
}
    ...
    if (!bDX9Hooked && !bRendererFound) {
        ...
        if (library != nullptr) {
            ...
            if (Direct3DCreate9Ex != nullptr) {
                ...
                pD3D->CreateDeviceEx(D3DADAPTER_DEFAULT, D3DDEVTYPE_NULLREF, NULL, D3DCREATE_HARDWARE_VERTEXPROCESSING,
                                     &params, NULL, reinterpret_cast<IDirect3DDevice9Ex **>(&pDevice));
            } else {
                decltype(Direct3DCreate9) *Direct3DCreate9 =
                    (decltype(Direct3DCreate9))GetProcAddress(library, "Direct3DCreate9");
    ...
    if (!bDX10Hooked && !bRendererFound) {
        ...
        if (library != nullptr) {
            ...
            if (D3D10CreateDeviceAndSwapChain != nullptr) {
                ...
                D3D10CreateDeviceAndSwapChain(NULL, D3D10_DRIVER_TYPE_NULL, NULL, 0, D3D10_SDK_VERSION, &SwapChainDesc,
                                              &pSwapChain, &pDevice);
            }
        }
    ...
    if (!bDX11Hooked && !bRendererFound) {
        ...
        if (library != nullptr) {
            ...
            if (D3D11CreateDeviceAndSwapChain != nullptr) {
                ...
                D3D11CreateDeviceAndSwapChain(NULL, D3D_DRIVER_TYPE_NULL, NULL, 0, NULL, NULL, D3D11_SDK_VERSION,
                                              &SwapChainDesc, &pSwapChain, &pDevice, NULL, NULL);
            }
        }
        if (pSwapChain != nullptr) {
            PRINT_DEBUG("Hooked IDXGISwapChain::Present to detect DX Version\n");
                        ...
                        if (dxgi != nullptr) {
                            ...
                            if (CreateDXGIFactory1 != nullptr) {
                                ...
                                if (pDXGIFactory != nullptr) {
                                    pDXGIFactory->CreateSwapChainForHwnd(pCommandQueue, DummyHWnd, &SwapChainDesc, NULL,
                                                                         NULL, &pSwapChain);
                                }
                            }
                        }
...
void RendererDetector::FindRendererProc(RendererDetector *_this) {
    ...
    std::vector<std::string> const libraries = {OPENGL_DLL, DX12_DLL, DX11_DLL, DX10_DLL, DX9_DLL};
...
RendererDetector::RendererDetector()
    : HookThread(nullptr), HookRetries(0), bRendererFound(false), bDX9Hooked(false), bDX10Hooked(false),
      bDX11Hooked(false), bDX12Hooked(false), bDXGIHooked(false), bOGLHooked(false), RenderDetectHook(nullptr),
      GameRenderer(nullptr), Atom(0), DummyHWnd(nullptr) {}

...
BaseHook *RendererDetector::GetRenderer() const { return GameRenderer; }
...
void RendererDetector::CreateHWnd() {
  if (DummyHWnd == nullptr) {
    ...
    if (Atom > 0) {
      DummyHWnd =
          ::CreateWindowEx(NULL, windowClassName, "", WS_OVERLAPPEDWINDOW, 0, 0,
                           1, 1, NULL, NULL, hInst, nullptr);
...
HRESULT WINAPI RendererDetector::MyIDXGISwapChain_Present(IDXGISwapChain *_this,
                                                          UINT SyncInterval,
                                                          UINT Flags) {
  ...
  if (!inst.StopRetry()) {
    ...
    if (inst.bDX12Hooked) {
      _this->GetDevice(
          IID_PPV_ARGS(reinterpret_cast<ID3D12Device **>(&pDevice)));
    }
    if (pDevice) {
      ...
      if (inst.bDX11Hooked) {
        _this->GetDevice(
            IID_PPV_ARGS(reinterpret_cast<ID3D11Device **>(&pDevice)));
      }
      if (pDevice) {
        ...
        if (inst.bDX10Hooked) {
          _this->GetDevice(
              IID_PPV_ARGS(reinterpret_cast<ID3D10Device **>(&pDevice)));
        }
        if (pDevice) {
...
HRESULT WINAPI RendererDetector::MyPresent(IDirect3DDevice9 *_this,
  ...
  RendererDetector &inst = RendererDetector::Instance();
  HookManager &hm = HookManager::Instance();
  auto res = (_this->*_IDirect3DDevice9_Present)(
...
HRESULT WINAPI RendererDetector::MyPresentEx(
    IDirect3DDevice9Ex *_this, const RECT *pSourceRect, const RECT *pDestRect,
    HWND hDestWindowOverride, const RGNDATA *pDirtyRegion, DWORD dwFlags) {
  RendererDetector &inst = RendererDetector::Instance();
  HookManager &hm = HookManager::Instance();
  auto res = (_this->*_IDirect3DDevice9Ex_PresentEx)(
...
void RendererDetector::HookDXGIPresent(IDXGISwapChain *pSwapChain) {
  if (!bDXGIHooked) {
    bDXGIHooked = true;
    (void *&)_IDXGISwapChain_Present = (*reinterpret_cast<void ***>(
        pSwapChain))[(int)IDXGISwapChainVTable::Present];

    RenderDetectHook->BeginHook();
    RenderDetectHook->HookFuncs(std::pair<void **, void *>(
        (PVOID *)&_IDXGISwapChain_Present,
        &RendererDetector::MyIDXGISwapChain_Present));
    RenderDetectHook->EndHook();
  }
}

void RendererDetector::HookDX9Present(IDirect3DDevice9 *pDevice, bool ex) {
  (void *&)_IDirect3DDevice9_Present = (*reinterpret_cast<void ***>(
      pDevice))[(int)IDirect3DDevice9VTable::Present];
  if (ex)
    (void *&)_IDirect3DDevice9Ex_PresentEx = (*reinterpret_cast<void ***>(
        pDevice))[(int)IDirect3DDevice9VTable::PresentEx];

  RenderDetectHook->BeginHook();
  RenderDetectHook->HookFuncs(std::pair<void **, void *>(
      (PVOID *)&_IDirect3DDevice9_Present, &RendererDetector::MyPresent));
  if (ex)
    RenderDetectHook->HookFuncs(
        std::pair<void **, void *>((PVOID *)&_IDirect3DDevice9Ex_PresentEx,
                                   &RendererDetector::MyPresentEx));
  RenderDetectHook->EndHook();
}

void RendererDetector::HookwglMakeCurrent(
    decltype(wglMakeCurrent) *wglMakeCurrent) {
  _wglMakeCurrent = wglMakeCurrent;
  RenderDetectHook->BeginHook();
  RenderDetectHook->HookFuncs(std::pair<void **, void *>(
      (PVOID *)&_wglMakeCurrent, &RendererDetector::MywglMakeCurrent));
  RenderDetectHook->EndHook();
}
...
void RendererDetector::HookDX9() {
  if (!bDX9Hooked && !bRendererFound) {
    ...
    if (library != nullptr) {
      Direct3DCreate9Ex = (decltype(Direct3DCreate9Ex))GetProcAddress(
          library, "Direct3DCreate9Ex");
      ...
      if (Direct3DCreate9Ex != nullptr) {
        ...
        pD3D->CreateDeviceEx(D3DADAPTER_DEFAULT, D3DDEVTYPE_NULLREF, NULL,
                             D3DCREATE_HARDWARE_VERTEXPROCESSING, &params, NULL,
                             reinterpret_cast<IDirect3DDevice9Ex **>(&pDevice));
      } else {
        decltype(Direct3DCreate9) *Direct3DCreate9 =
            (decltype(Direct3DCreate9))GetProcAddress(library,
                                                      "Direct3DCreate9");
        if (Direct3DCreate9 != nullptr) {
          pD3D = reinterpret_cast<IDirect3D9Ex *>(
              Direct3DCreate9(D3D_SDK_VERSION));
          pD3D->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_NULLREF, NULL,
    ...
    if (pDevice != nullptr) {
      ...
      h->LoadFunctions(reinterpret_cast<IDirect3DDevice9 *>(pDevice),
                       Direct3DCreate9Ex != nullptr);
      HookManager::Instance().AddHook(h);
      HookDX9Present(reinterpret_cast<IDirect3DDevice9 *>(pDevice),
                     Direct3DCreate9Ex != nullptr);
    } else {
  ...
  if (!bDX10Hooked && !bRendererFound) {
    ...
    if (library != nullptr) {
      decltype(D3D10CreateDeviceAndSwapChain) *D3D10CreateDeviceAndSwapChain =
          (decltype(D3D10CreateDeviceAndSwapChain))GetProcAddress(
              library, "D3D10CreateDeviceAndSwapChain");
      if (D3D10CreateDeviceAndSwapChain != nullptr) {
        ...
        D3D10CreateDeviceAndSwapChain(NULL, D3D10_DRIVER_TYPE_NULL, NULL, 0,
    ...
    if (pSwapChain != nullptr) {
      ...
      PRINT_DEBUG(
          "Failed to Hook IDXGISwapChain::Present to detect DX Version\n");
    }
    if (pDevice)
      pDevice->Release();
    if (pSwapChain)
  ...
  if (!bDX11Hooked && !bRendererFound) {
    ...
    if (library != nullptr) {
      decltype(D3D11CreateDeviceAndSwapChain) *D3D11CreateDeviceAndSwapChain =
          (decltype(D3D11CreateDeviceAndSwapChain))GetProcAddress(
              library, "D3D11CreateDeviceAndSwapChain");
      if (D3D11CreateDeviceAndSwapChain != nullptr) {
        ...
        D3D11CreateDeviceAndSwapChain(NULL, D3D_DRIVER_TYPE_NULL, NULL, 0, NULL,
                                      NULL, D3D11_SDK_VERSION, &SwapChainDesc,
                                      &pSwapChain, &pDevice, NULL, NULL);
      }
    }
    if (pSwapChain != nullptr) {
      PRINT_DEBUG("Hooked IDXGISwapChain::Present to detect DX Version\n");
  ...
  if (!bDX12Hooked && !bRendererFound) {
    ...
    if (library != nullptr) {
      decltype(D3D12CreateDevice) *D3D12CreateDevice =
          (decltype(D3D12CreateDevice))GetProcAddress(library,
                                                      "D3D12CreateDevice");
          ...
          if (pCommandQueue != nullptr) {
            ...
            if (dxgi != nullptr) {
              decltype(CreateDXGIFactory1) *CreateDXGIFactory1 =
                  (decltype(CreateDXGIFactory1))GetProcAddress(
                      dxgi, "CreateDXGIFactory1");
              if (CreateDXGIFactory1 != nullptr) {
                CreateDXGIFactory1(IID_PPV_ARGS(&pDXGIFactory));
                if (pDXGIFactory != nullptr) {
                  pDXGIFactory->CreateSwapChainForHwnd(pCommandQueue, DummyHWnd,
                                                       &SwapChainDesc, NULL,
                                                       NULL, &pSwapChain);
                }
              }
...
void RendererDetector::HookOGL() {
  if (!bOGLHooked && !bRendererFound) {
    ...
    if (library != nullptr) {
      wglMakeCurrent =
          (decltype(wglMakeCurrent))GetProcAddress(library, "wglMakeCurrent");
      wglSwapBuffers =
          (decltype(wglSwapBuffers))GetProcAddress(library, "wglSwapBuffers");
    }
...
void RendererDetector::FindRendererProc(RendererDetector *_this) {
  ...
  std::vector<std::string> const libraries = {OPENGL_DLL, DX12_DLL, DX11_DLL,
                                              DX10_DLL, DX9_DLL};
...
RendererDetector::RendererDetector()
    : HookThread(nullptr), HookRetries(0), bRendererFound(false),
      bDX9Hooked(false), bDX10Hooked(false), bDX11Hooked(false),
      bDX12Hooked(false), bDXGIHooked(false), bOGLHooked(false),
      RenderDetectHook(nullptr), GameRenderer(nullptr), Atom(0),
      DummyHWnd(nullptr) {}

...
void BlackMarketDetours::SetupDetours() {
    auto PrizeWheelComponent = UVAL_PrizeWheelComponent::StaticClass();
    if (PrizeWheelComponent) {
        auto OnClientInteract = PrizeWheelComponent->GetFunction("VAL_PrizeWheelComponent", "OnClientInteract");
        if (OnClientInteract) {
            if (OnClientInteract->ExecFunction != &Func_OnClientInteract) {
                Orig_OnClientInteract = OnClientInteract->ExecFunction;
                OnClientInteract->ExecFunction = &Func_OnClientInteract;
            }
        }
        auto SpinTheWheel = PrizeWheelComponent->GetFunction("VAL_PrizeWheelComponent", "RpcNetMulticast_SpinTheWheel");
        if (SpinTheWheel) {
            if (SpinTheWheel->ExecFunction != &Func_SpinTheWheel) {
                Orig_SpinTheWheel = SpinTheWheel->ExecFunction;
                SpinTheWheel->ExecFunction = &Func_SpinTheWheel;
            }
        }
        auto IsInteractable = PrizeWheelComponent->GetFunction("VAL_PrizeWheelComponent", "IsInteractable");
        if (IsInteractable) {
            if (IsInteractable->ExecFunction != &Func_IsInteractable) {
                Orig_IsInteractable = IsInteractable->ExecFunction;
                IsInteractable->ExecFunction = &Func_IsInteractable;
            }
        }
    }
...
void BlackMarketDetours::Func_OnClientInteract(void* Context, void* TheStack, void* Result) {
    ...
    if (Orig_OnClientInteract) // this shouldn't be void lol
    {
        Orig_OnClientInteract(Context, TheStack, Result);
    }
...
void BlackMarketDetours::Func_SpinTheWheel(void* Context, void* TheStack, void* Result) {
    ...
    if (Orig_SpinTheWheel) // this still shouldn't happen
    {
        Orig_SpinTheWheel(Context, TheStack, Result);
    }
}

void BlackMarketDetours::Func_IsInteractable(void* Context, void* TheStack, void* Result) {
    if (Orig_IsInteractable) // better safe than sorry
    {
        Orig_IsInteractable(Context, TheStack, Result);
    }

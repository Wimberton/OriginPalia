...
template <typename UEType = UObject> std::vector<UEType *> FindObjectsOfType() {
    ...
    for (int i = 0; i < UObject::GObjects->Num(); ++i) {
        ...
        if (!Obj || !Obj->IsValidLowLevel() ||
            Obj->IsDefaultObject()) // TODO: Figure out why sometimes we get 0xffffffffffffffff
            continue;
...
template <typename UEType = AActor> std::vector<UEType *> FindActorsOfType(UWorld *WorldContext) {
...
template <typename UEType = AActor> std::vector<UEType *> FindAllActorsOfType(UWorld *WorldContext) {
    std::vector<UEType *> Actors;
    ...
    static_cast<UGameplayStatics *>(UGameplayStatics::StaticClass()->DefaultObject)
        ->GetAllActorsOfClass(WorldContext, Class, &WorldActors);
    for (int i = 0; i < WorldActors.Num(); ++i) {
        AActor *Actor = WorldActors[i];
...
template <typename UEType> class TWeakObjectPtr : public FWeakObjectPtr {
    public:
        TWeakObjectPtr() : FWeakObjectPtr() {}

        explicit TWeakObjectPtr(UEType *Object) { *this = Object; }

        TWeakObjectPtr(const TWeakObjectPtr &Other) : FWeakObjectPtr(Other) {}

        template <typename OtherType> explicit TWeakObjectPtr(const TWeakObjectPtr<OtherType> &Other) {
            *this = Other.Get();
        }

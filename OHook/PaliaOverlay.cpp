...
void PaliaOverlay::SetupColors() {
    ...
    for (auto& AnimalColor : AnimalColors) {
        AnimalColor[static_cast<int>(ECreatureQuality::Tier1)] = IM_COL32(0x80, 0x80, 0x80, 0xFF); // Light Gray
    ...
    for (auto& BugColor : BugColors) {
        BugColor[static_cast<int>(EBugQuality::Common)] = IM_COL32(0x80, 0x80, 0x80, 0xFF);
    ...
    SingleColors[static_cast<int>(EOneOffs::Treasure)] = IM_COL32(0xFF, 0xD7, 0x00, 0xFF);  // bright gold
    SingleColors[static_cast<int>(EOneOffs::Others)] = IM_COL32(0xC0, 0xC0, 0xC0, 0xFF); // washed gray
...
void PaliaOverlay::DrawHUD() {
    ...
    std::string watermarkText = "OriginPalia - discord.gg/OriginSoftware - Game V0.181.0";
    ...
    if (PlayerController) {
        ...
        if (ValeriaController) {
            gDetourManager.SetupDetour(ValeriaController->GetTrackingComponent());
        }
        
        AValeriaCharacter* ValeriaCharacter = GetValeriaCharacter();
        if (ValeriaCharacter) {
            // VPLAYER CONTROLLER COMPONENT
            gDetourManager.SetupDetour(ValeriaCharacter->GetValeriaPlayerController());

            // MOVEMENT COMPONENT
            gDetourManager.SetupDetour(ValeriaCharacter->GetValeriaCharacterMovementComponent());

            // VILLAGER STORE COMPONENT
            gDetourManager.SetupDetour(ValeriaCharacter->StoreComponent);

            // ITEM UPGRADE COMPONENT
            gDetourManager.SetupDetour(ValeriaCharacter->GetItemUpgradeComponent());
        
            // INVENTORY COMPONENT
            gDetourManager.SetupDetour(ValeriaCharacter->GetInventory());
        
            // EQUIPMENT COMPONENT
            gDetourManager.SetupDetour(ValeriaCharacter->GetEquipment());
        
            // FIRING COMPONENT
            gDetourManager.SetupDetour(ValeriaCharacter->GetFiringComponent());
        
            // FISHING COMPONENT
            gDetourManager.SetupDetour(ValeriaCharacter->GetFishing());
        
            // PLACEMENT COMPONENT
            gDetourManager.SetupDetour(ValeriaCharacter->GetPlacement());

        }
        
        // HOOKING PROCESSEVENT IN AHUD
        if (HookedClient != PlayerController->MyHUD) {
...
void PaliaOverlay::DrawOverlay() {
    ...
    const auto WindowTitle = std::string("OriginPalia V2.7.5");
    auto Overlay = static_cast<PaliaOverlay*>(Instance);
    ...
    if (ImGui::Begin(WindowTitle.data(), &show, window_flags)) {
        ...
        if (OpenTab == 0) {
            ...
            if (ImGui::CollapsingHeader("Visual Settings##VisualSettingsGeneralHeader", ImGuiTreeNodeFlags_DefaultOpen)) {
                ...
                /*if (ImGui::Checkbox("Use Icons", &Configuration::bESPIcons)) {
                    Configuration::Save();
                }
                if (ImGui::IsItemHovered())
                    ImGui::SetTooltip("Display icons instead of text on screen");
                if (Configuration::bESPIcons) {
                    if (ImGui::Checkbox("Show Distance with Icons", &Configuration::bESPIconDistance)) {
                        Configuration::Save();
                    }
                }*/
            ...
            if (ImGui::CollapsingHeader("Player & Entities##PlayerEntitiesSettingHeader")) {
                if (ImGui::Button("Toggle All##MiscBtn")) {
                    ...
                    Singles[static_cast<int>(EOneOffs::Treasure)] = newState;
                ...
                ImGui::BeginTable("Odds", 3);
                {
                    ...
                    ImGui::Text("Treasure");
                    ImGui::TableNextColumn();
                    if (ImGui::Checkbox("##TreasureChests", &Singles[static_cast<int>(EOneOffs::Treasure)])) {
                        Configuration::Save();
                    }
                    ImGui::TableNextColumn();
                    ImGui::ColorPicker("##TreasureChests", &SingleColors[static_cast<int>(EOneOffs::Treasure)]);
                    ImGui::TableNextRow();
                    ImGui::TableNextColumn();
                    ImGui::Text("Stables");
                    ImGui::TableNextColumn();
        ...
        // ==================================== 1 Aimbots & Fun TAB
            ...
            if (ImGui::CollapsingHeader("Aimbot Settings##AimbotSettingsHeader", ImGuiTreeNodeFlags_DefaultOpen)) {
                if (ValeriaCharacter) {
                    if (Configuration::bDrawFOVCircle) {
                        ...
                        if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled)) ImGui::SetTooltip("Teleport Bug Bombs & Arrows To Your Target (Animals, Max 22m)");
            ...
            if (ImGui::CollapsingHeader("Fun Settings##FunModsHeader", ImGuiTreeNodeFlags_DefaultOpen)) {
                if (ValeriaCharacter) {
                    ...
                    if (ImGui::Checkbox("Vanish Mode", &Configuration::bEnableVanishMode)) {
                        Configuration::Save();
                    }
                    if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled)) ImGui::SetTooltip("Make yourself invisible when this is enabled (client-only).");
                    ...
                    if (ImGui::Checkbox("Further Interaction Distance", &Configuration::bEnableInteractionMods)) {
                        Configuration::Save();
                    }
                    if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled)) ImGui::SetTooltip("Interact with things from further distances");
                    ...
                    if (Configuration::bEnableInteractionMods) {
                        ...
                        if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled)) ImGui::SetTooltip("Adjust the maximum distance for interactions. Range: 1.0 to 99999.0");
                    }
                }
                else {
                    ImGui::Text("Waiting for character initialization...");
        ...
        // ==================================== 2 Movement & Teleport TAB
            ...
            if (ImGui::CollapsingHeader("Locations & Coordinates##LocationSettingsHeader", ImGuiTreeNodeFlags_DefaultOpen)) {
                if (ValeriaCharacter) {
                    ...
                    if (ImGui::ListBoxHeader("##TeleportList", ImVec2(-1, 150))) {
        ...
        // ==================================== 3 Selling & Player TAB
            ...
            if (ImGui::CollapsingHeader("Villager Guild Stores##GuildStoresHeader", ImGuiTreeNodeFlags_DefaultOpen)) {
                if (ValeriaCharacter) {
                    ...
                    if (Overlay->GameplayUIManager && Overlay->GameplayUIManager->IsValidLowLevel()) {
                        ImGui::SameLine();
                        if (ImGui::Button("Open Guild Store##OpenGuildStoreBtn")) {
                            switch (selectedStore) {
                            case 0:
                                gDetourManager.Func_DoOpenGuildStore(this, ValeriaCharacter, 2 /*Einar's Fishing Guild store*/);
                                show = false;
                                break;
                            case 1:
                                gDetourManager.Func_DoOpenGuildStore(this, ValeriaCharacter, 3 /*Auni's Bug Guild store*/);
                                show = false;
                                break;
                            case 2:
                                gDetourManager.Func_DoOpenGuildStore(this, ValeriaCharacter, 6 /*Ashura's Foraging Guild store*/);
                                show = false;
                                break;
                            case 3:
                                gDetourManager.Func_DoOpenGuildStore(this, ValeriaCharacter, 7 /*Hassian's Hunting Guild store*/);
                                show = false;
                                break;
                            case 4:
                                gDetourManager.Func_DoOpenGuildStore(this, ValeriaCharacter, 10 /*Reth's Cooking Guild store*/);
                                show = false;
                                break;
                            case 5:
                                gDetourManager.Func_DoOpenGuildStore(this, ValeriaCharacter, 12 /*Tish's Furniture Guild store*/);
                                show = false;
                                break;
                            case 6:
                                gDetourManager.Func_DoOpenGuildStore(this, ValeriaCharacter, 8806 /*Hodari's Mining Guild store*/);
                                show = false;
                                break;
                            case 7:
                                gDetourManager.Func_DoOpenGuildStore(this, ValeriaCharacter, 19928 /*Badruu's Gardening Guild store*/);
                                show = false;
                                break;
                            default:
                    ...
                    /*
                    * BUGGY - Needs more testing, crashes on trying to load configs not loaded into memory already (Steam issue? test on standalone)
                    * 
                    static std::string selectedConfigStore;
                    static std::map<std::string, std::string> configStores = {
                        {"Black Market", "/Game/Configs/RegisterStores/DA_Store_BlackMarket.DA_Store_BlackMarket"},
                        {"Blacksmith", "/Game/Configs/RegisterStores/DA_Store_Blacksmith.DA_Store_Blacksmith"},
                        {"City Hall", "/Game/Configs/RegisterStores/DA_Store_Housing.DA_Store_Housing"},
                        {"Farm", "/Game/Configs/RegisterStores/DA_Store_Crops.DA_Store_Crops"},
                        {"General Store", "/Game/Configs/RegisterStores/DA_Store_General.DA_Store_General"},
                        {"Furniture", "/Game/Configs/RegisterStores/DA_Store_Furniture.DA_Store_Furniture"},
                        {"Inn", "/Game/Configs/RegisterStores/DA_Store_Inn.DA_Store_Inn"},
                        {"Library", "/Game/Configs/RegisterStores/DA_Store_Library.DA_Store_Library"},
                        {"Spooky Moon Store", "/Game/Configs/RegisterStores/DA_Store_Halloween.DA_Store_Halloween"},
                        {"Winterlights Store", "/Game/Configs/RegisterStores/DA_Store_Winter.DA_Store_Winter"},
                    };
                    ImGui::PushItemWidth(width - spacing * 2.0f - buttonSize * 2.0f); //ImGui::PushItemWidth(CalcTextSize(stores[2])).x + style.FramePadding.x * 2.0f);
                    ImGui::SetNextItemWidth(250.0f);
                    if (ImGui::BeginCombo("##BuildingStores", selectedConfigStore.c_str())) {
                        for (std::map<std::string, std::string>::iterator it = configStores.begin(); it != configStores.end(); ++it)
                        {
                            const std::string configStore = it->first;
                            const bool is_selected = (selectedConfigStore == configStore);
                            if (ImGui::Selectable(configStore.c_str(), is_selected)) {
                                selectedConfigStore = configStore;
                            }
                            if (is_selected)
                                ImGui::SetItemDefaultFocus();
                        }
                    ...
                    if (Overlay->GameplayUIManager && Overlay->GameplayUIManager->IsValidLowLevel()) {
                        ...
                        if (ImGui::Button("Open Building Store##OpenBuildingStoreBtn")) {
                            auto test = configStores[selectedConfigStore];
                            gDetourManager.Func_DoOpenConfigStore(this, ValeriaCharacter, test);
                            show = false;
                        }
                    }
                    else {
            ...
            if (ImGui::CollapsingHeader("Player Features##PlayerSettingsHeader", ImGuiTreeNodeFlags_DefaultOpen)) {
                if (ValeriaCharacter) {
                    ...
                    if (Configuration::bEasyModeActive) {
                        ImGui::SameLine();
                        ImGui::Text("EASY MODE - ON");
                    }
                    else {
                        ImGui::SameLine();
                        ImGui::Text("EASY MODE - OFF");
                    }

                    if (Overlay->GameplayUIManager && Overlay->GameplayUIManager->IsValidLowLevel()) {
                        if (ImGui::Button("Open Player Storage##OpenPlayerStorageBtn")) {
                            GameplayUIManager->Implementation_OpenPlayerStorage();
                            show = false;
                        }
                    }
                    else {
                        ImGui::Text("Failed loading widget.");
                    }

                    if (Overlay->GameplayUIManager && Overlay->GameplayUIManager->IsValidLowLevel()) {
                        if (ImGui::Button("Open CVars Menu##OpenCVarsMenuBtn")) {
                            GameplayUIManager->Implementation_OpenCVarMenu();
                            show = false;
                        }
                    }
                    else {
        ...
        // ==================================== 4 Skills & Tools TAB
            ...
            if (ImGui::CollapsingHeader("Tool Settings##ToolsHeader", ImGuiTreeNodeFlags_DefaultOpen)) {
                if (ValeriaCharacter) {
                    ...
                    if (ImGui::Checkbox("Enable AutoGather", &Configuration::bEnableAutoGather)) {
                        Configuration::Save();
                    }
                    if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled)) ImGui::SetTooltip("Automatically gather forageables (within range) as you play.");
                }
                else {
                    ImGui::Text("Waiting for character initialization...");
                }
            ...
            if (ImGui::CollapsingHeader("Fishing Settings##FishingHeader", ImGuiTreeNodeFlags_DefaultOpen)) {
                if (FishingComponent) {
                    if (ImGui::Checkbox("No Rod Durability Loss", &Configuration::bFishingNoDurability)) {
                        Configuration::Save();
                    }
                    ...
                    if (ImGui::Combo("##FishingSpotsCombo", &bSelectedFishingSpot, bFishingSpots, IM_ARRAYSIZE(bFishingSpots))) {
                        if (bSelectedFishingSpot > 0) {
                            sOverrideFishingSpot = UKismetStringLibrary::Conv_StringToName(bFishingSpotsFString[bSelectedFishingSpot - 1]);
                        }
                        else {
                            bSelectedFishingSpot = 0;
                            sOverrideFishingSpot = {};
...
void PaliaOverlay::ProcessActors(int step) {
    ...
    case EType::Tree:
        if (AnyTrue2D(Trees)) {
            SearchClasses.push_back(ABP_ValeriaGatherableLoot_Lumber_C::StaticClass());
        }
        break;
    case EType::Ore:
        if (AnyTrue2D(Ores)) {
            SearchClasses.push_back(ABP_ValeriaGatherableLoot_Mining_Base_C::StaticClass());
        }
        break;
    case EType::Bug:
        if (AnyTrue3D(Bugs)) {
            SearchClasses.push_back(ABP_ValeriaBugCatchingCreature_C::StaticClass());
        }
        break;
    case EType::Animal:
        if (AnyTrue2D(Animals)) {
            SearchClasses.push_back(ABP_ValeriaHuntingCreature_C::StaticClass());
        }
        break;
    case EType::Forage:
        if (AnyTrue2D(Forageables)) {
            SearchClasses.push_back(ABP_Valeria_Gatherable_Placed_C::StaticClass());
        }
        break;
    case EType::Fish:
        if (AnyTrue(Fish)) {
            STATIC_CLASS("BP_WaterPlane_Fishing_Base_SQ_C")
            STATIC_CLASS("BP_Minigame_Fish_C")
        }
        break;
    case EType::Loot:
        if (Singles[static_cast<int>(EOneOffs::Loot)]) {
            SearchClasses.push_back(ABP_Loot_C::StaticClass());
        }
        break;
    case EType::Stables:
        if (Singles[static_cast<int>(EOneOffs::Stables)]) {
            SearchClasses.push_back(ABP_Stables_Sign_C::StaticClass());
        }
        break;
    case EType::Players:
    ...
    case EType::Quest:
        if (Singles[static_cast<int>(EOneOffs::Quest)]) {
            SearchClasses.push_back(ABP_SimpleInspect_Base_C::StaticClass());
            SearchClasses.push_back(ABP_QuestInspect_Base_C::StaticClass());
            SearchClasses.push_back(ABP_QuestItem_BASE_C::StaticClass());
        }
        break;
    case EType::RummagePiles:
        if (Singles[static_cast<int>(EOneOffs::RummagePiles)]) {
            STATIC_CLASS("BP_BeachPile_C")
            STATIC_CLASS("BP_ChapaaPile_C")
        }
        break;
    case EType::Gates:
        SearchClasses.push_back(ABP_Stables_FrontGate_01_C::StaticClass());
        SearchClasses.push_back(ABP_Stables_FrontGate_02_C::StaticClass());
        SearchClasses.push_back(ABP_Stables_FrontGate_03_C::StaticClass());
        break;
    case EType::Treasure:
        if (Singles[static_cast<int>(EOneOffs::Treasure)]) {
            SearchClasses.push_back(ABP_WorldPersistGatherable_Treasure_Base_C::StaticClass());
        }
        break;
    default:
        break;
    ...
    if (ActorType == EType::Fish || ActorType == EType::Quest || ActorType == EType::RummagePiles || ActorType == EType::Gates || ActorType == EType::Treasure) {
    ...
    for (AActor* Actor : Actors) {
        ...
        if (ActorPosition.IsZero() || (ActorPosition.X == 2.0 && ActorPosition.Y == 0.0))
            continue;
        ...
        double Distance = PlayerController->K2_GetPawn()->K2_GetActorLocation().GetDistanceTo(ActorPosition);
        if (Name.empty()) {
        ...
        CachedActors.push_back({ Actor, ActorPosition, Name, ActorType, Type, Quality, Variant, Distance, Texture });
    }
}

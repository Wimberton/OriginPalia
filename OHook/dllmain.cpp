...
DWORD WINAPI MainThread(const LPVOID lpReserved) {
    ...
    if (reinterpret_cast<uintptr_t>(GetModuleHandle("steam_api64.dll")) ||
        reinterpret_cast<uintptr_t>(GetModuleHandle("steamclient64.dll"))) {
        IsSteamVersion = true;
    } else {
        char fileName[MAX_PATH];
        GetModuleFileName(nullptr, fileName, MAX_PATH);
        if (const std::string filePath(fileName);
            filePath.find("PaliaClientSteam-Win64-Shipping.exe") != std::string::npos) {
            IsSteamVersion = true;
        }
    }
